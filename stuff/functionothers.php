...
{
$qr1= mysql_query("SELECT id FROM test_image where email='$k'") or die(mysql_error());
$row=mysql_fetch_assoc($qr1);
$ids=$row['id'];
echo "<img src=/bsnl/image.php?id=$ids width=310 height=310 />";
}

function getfloorothers($k)
{


$qr2=mysql_query("SELECT floor FROM employee1 where email='$k'") or die(mysql_error()) ;
